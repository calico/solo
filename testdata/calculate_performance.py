...
def main():
    ...
    performance_tracking["date (dt)"] = pd.to_datetime(
        performance_tracking["date"], format="%Y-%m-%d %H"
    )
    for experiment_name, group in performance_tracking.groupby("experiment_name"):
        ...
        second_to_last, most_recent = (
            group["date (dt)"].drop_duplicates().sort_values()[-2:]
        )
        ...
        for metric in ["AUROC", "average_precision"]:
            mean_change = (
                most_recent_df[metric].mean() - second_to_last_df[metric].mean()
            )
            pvalue = mannwhitneyu(
                most_recent_df[metric], second_to_last_df[metric]
            ).pvalue
            print(f"Mean {metric} has changed by for {experiment_name}: {mean_change}")
            print(
                f"P value for metric change {metric} in experiment {experiment_name}: {pvalue}"
            )
            if mean_change < 0 and pvalue < 0.05:
                for x in range(0, 5):
                    print("WARNING!")
                print(
                    f"WARNING {metric} HAS GOTTEN SIGNIFICANTLY WORSE for {experiment_name}!"
                )
            if mean_change > 0 and pvalue < 0.05:
                for x in range(0, 5):
                    print("NICE JOB!")
                print(
                    f"NICE JOB {metric} HAS GOTTEN SIGNIFICANTLY BETTER for {experiment_name}!"
                )
