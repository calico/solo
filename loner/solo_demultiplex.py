...
def _calculate_probabilities(z, total_samples):
    ...
    for x in np.arange(num_of_barcodes):
        ...
        # get parameters of disti=ribution, assuming lognormal
...
def _get_clusters(clustering_data: AnnData,
                  resolutions: list):
    '''
    '''
...
def _calculate_llrts(data, doublet_llr_threshold, negative_llr_threshold, total_samples):
    '''
    '''
    probabilities_for_each_hypothesis, all_indices, insilico_probs, counter_to_barcode_combo = calculate_probabilities(data, total_samples)
...
def solo_demultiplex(adata: AnnData,
                     negative_p_value: float = .001,
                     doublet_p_value: float = .25,
                     total_samples: int = 10000,
                     pre_existing_clusters: str = None,
                     inplace: bool = True,
                     resolutions: list = [.1, .25, .5, .75, .9, 1, 2, 5],
                     clustering_data: AnnData = None,):
    ...
    if clustering_data is not None:
        ...
        assert clustering_data.shape[0] == adata.shape[0]
        adata.obs["best_leiden"] = _get_clusters(clustering_data, resolutions)

    negative_llr_threshold = 100 - negative_p_value
    doublet_llr_threshold = 100 - doublet_p_value
    data = adata.X
    num_of_cells = adata.shape[0]
    results = pd.DataFrame(np.zeros((adata.shape[0], 7)),
                           columns=['llrt_d_s',
                                    'llrt_n_s',
                                    'dubs',
                                    'negs',
...
def plot_qc_checks_cell_hashing(adata: AnnData,
                                alpha: float = .05,
                                fig_path: str = None):
